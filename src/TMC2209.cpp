...
static TMC2209* uart0_instance = nullptr;
static TMC2209* uart1_instance = nullptr;

static void uart0_rx_handler() {
    if (uart0_instance) {
        uart0_instance->handleRxInterrupt();
    }
}

static void uart1_rx_handler() {
    if (uart1_instance) {
        uart1_instance->handleRxInterrupt();
    }
}
...
TMC2209::TMC2209()
{
    ...
    rx_queue_ = xQueueCreate(50, sizeof(uint8_t));
}

void TMC2209::setup(uart_inst_t * uart_instance,
    uint tx_pin,
    uint rx_pin,
...
// Replace serial methods with Pico SDK UART functions
...
size_t TMC2209::serialWrite(uint8_t c)
{
    ...
    {
        uart_putc_raw(uart_instance_, c);
        return 1;
    }
    return 0;
}
...
{
    uint8_t byte;
    if (xQueueReceive(rx_queue_, &byte, 0) == pdTRUE) {
        return byte;
    }
    return -1;
}
...
bool TMC2209::isEnabled()
{
    return gpio_get(hardware_enable_pin_) == false;
}
...
void TMC2209::handleRxInterrupt() {
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;

    while (uart_is_readable(uart_instance_)) {
        uint8_t byte = uart_getc(uart_instance_);
        xQueueSendFromISR(rx_queue_, &byte, &xHigherPriorityTaskWoken);
    }
